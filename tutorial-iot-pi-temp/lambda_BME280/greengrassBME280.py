...
def greengrass_hello_world_run():
    ...
    readout = bme280.readBME280()
    
    try:
        if not my_platform:
            client.publish(
                topic="hello/world", queueFullPolicy="AllOrException", payload=readout+"razi3 - Hello world! Sent from Greengrass Core."
            )
        else:
            client.publish(
                topic="hello/world",
                queueFullPolicy="AllOrException",
                payload=readout+"razi3 - Hello world! Sent from " "Greengrass Core running on platform: {}".format(my_platform),
            )
